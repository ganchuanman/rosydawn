...
#### Scenario: checkPort step
- **WHEN** 使用 `checkPort` processor
- **THEN** 系统检查指定端口是否被占用
- **THEN** 若端口被占用，返回可用替代端口列表

#### Scenario: cleanDist step
- **WHEN** 使用 `cleanDist` processor
- **THEN** 系统清理 dist 目录中的旧构建产物
- **THEN** 清理结果写入上下文

#### Scenario: compileProject step
- **WHEN** 使用 `compileProject` processor
- **THEN** 系统编译项目源代码
- **THEN** 编译产物和统计信息写入上下文

#### Scenario: optimizeAssets step
- **WHEN** 使用 `optimizeAssets` processor
- **THEN** 系统优化静态资源（压缩、混淆等）
- **THEN** 优化结果和大小统计写入上下文

#### Scenario: checkArticleStats step
- **WHEN** 使用 `checkArticleStats` processor
- **THEN** 系统统计文章数量、标签分布等信息
- **THEN** 统计结果写入上下文

#### Scenario: checkDeploymentStatus step
- **WHEN** 使用 `checkDeploymentStatus` processor
- **THEN** 系统检查最近一次部署时间和状态
- **THEN** 部署状态信息写入上下文
...
#### Scenario: buildProject step
- **WHEN** 使用 `buildProject` action
- **THEN** 系统执行项目构建（npm run build）
- **THEN** 构建产物输出到 dist 目录
- **THEN** 构建统计信息写入上下文

#### Scenario: executeDeploy step
- **WHEN** 使用 `executeDeploy` action
- **THEN** 系统执行部署脚本（npm run deploy）
- **THEN** 部署结果和 URL 写入上下文
...
#### Scenario: confirmDeploy step
- **WHEN** 使用 `confirmDeploy` notifier
- **THEN** 系统显示即将部署的信息，请求用户确认
- **THEN** 用户确认后继续，取消则中断流程
